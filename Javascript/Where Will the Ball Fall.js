...
var findBall = function(grid) {
    const balls = Array(grid[0].length).fill(-1);
    for(let i = 0; i < balls.length; ++i){
        let [x, y] = [i, 0];

        while(x < grid[0].length && y < grid.length && x > -1){
            let leftBorder = grid[y][x],
                currentBorder = grid[y][x + leftBorder];
            if(currentBorder !== leftBorder) break;
            if(leftBorder === -1){
                x--;
                y++;
